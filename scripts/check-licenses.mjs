...
function collectRuntimeDependencies() {
  ...
  const add = (name, range, origin) => {
    const entry = wanted.get(name) ?? { name, ranges: new Set(), origins: new Set() };
  ...
  for (const id of readdirSync(toolsDir).filter((d) => statSync(join(toolsDir, d)).isDirectory())) {
    ...
    for (const [name, range] of Object.entries(pkg.dependencies ?? {})) add(name, range, `tools/${id}`);
  }

  const web = JSON.parse(readFileSync(join(ROOT, 'apps', 'web', 'package.json'), 'utf8'));
  for (const [name, range] of Object.entries(web.dependencies ?? {})) add(name, range, 'apps/web');
...
lines.push('## Summary');
lines.push('');
lines.push('| Package | Version | Licence | Used by |');
lines.push('| --- | --- | --- | --- |');
for (const r of rows) {
  lines.push(`| [${r.name}](${r.homepage}) | ${r.version} | ${r.licence} | ${r.usedBy.join(', ')} |`);
}
lines.push('');
...
console.log(
  `Licence check passed. ${rows.length} runtime dependenc${rows.length === 1 ? 'y' : 'ies'}, all permissive, notices written to docs/THIRD-PARTY.md.`,
);
