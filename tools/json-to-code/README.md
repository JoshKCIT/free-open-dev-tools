...
Infers a type model from a pasted JSON sample and renders typed source text: a TypeScript interface, a Go struct, a Rust struct or a Python TypedDict class so far, checked directly against each language's own naming rules; the TypeScript output is also checked with the real TypeScript compiler under strict mode.

## Supported

- TypeScript, Go, Rust and Python output from the same inferred model
...
- Keys that are not valid identifiers, or that collide with a keyword, get an escaped name per language, with the original key kept in a struct tag, a rename attribute or the key text itself
- A generated type name that would shadow a language-provided type name gets a Type suffix

## Limits

- Output is source text only; no compiler, formatter or package manager runs for Go, Rust or Python, and none of their output is executed by this tool
