 ...
 * below. Fetched 2026-09-26 and vendored byte for byte at
...
export const COMPOSE_SPEC_SCHEMA: Readonly<Record<string, unknown>> = {
  ...
  properties: {
    ...
    jobs: {
      ...
      additionalProperties: false,
      description: 'Jobs are containers that run to completion.',
    },
  },
  patternProperties: {
    '^x-': {},
  ...
  $defs: {
    container_spec: {
      ...
      properties: {
        ...
        configs: {
          $ref: '#/$defs/service_config_or_secret',
    ...
    job: {
      ...
      properties: {
        profiles: {
          $ref: '#/$defs/list_of_strings',
          description:
            'List of profiles for this job. When profiles are specified, the job is only active when the profile is activated.',
        },
        triggers: {
          type: 'object',
          description:
          ...
          properties: {
            manual: {
              type: ['boolean', 'string'],
              description:
                'Whether the job can be triggered manually by an explicit run command. Defaults to true; an explicit false forbids manual execution.',
            },
            schedule: {
              type: 'array',
              description: 'List of schedules for the job.',
              items: {
                oneOf: [
                  {
                    type: 'string',
    ...
    schedule: {
      type: 'object',
      description: 'Schedule configuration for a job trigger.',
      required: ['cron'],
      properties: {
        cron: {
          type: 'string',
        ...
        timezone: {
          type: 'string',
          description:
            "Timezone used to evaluate the cron expression (e.g. 'Europe/Paris'). Defaults to the platform's local timezone.",
        },
        concurrency: {
          type: 'string',
          enum: ['forbid', 'queue'],
          description:
            "Policy applied when the schedule fires while a previous run is still in progress: prevent the new run ('forbid', the default) or queue it ('queue').",
        },
        missed_fires: {
          type: 'string',
          enum: ['one', 'skip'],
          description:
            "Policy applied to fires missed while the platform was unavailable: run a single catch-up ('one', the default) or skip them ('skip').",
        },
      },
      additionalProperties: false,
      patternProperties: {
    ...
    pre_start_hook: {
      ...
      properties: {
        per_replica: {
          type: ['boolean', 'string'],
          description: 'When true, the hook runs once per service replica instead of once per service.',
        },
      },
    },
    env_file: {
          ...
          oneOf: [
            ...
            },
            {
              ...
              properties: {
                ...
                no_cache_filter: {
                  $ref: '#/$defs/string_or_list',
                  description: 'Do not use build cache for the specified stages.',
                },
                additional_contexts: {
                  $ref: '#/$defs/list_or_dict',
