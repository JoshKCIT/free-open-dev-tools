/**
 * Default theme values transcribed from the pinned `tailwindcss` package's own
 * `theme.css` (the real source of truth this project ships against), never
 * from memory. `test/index.test.ts` re-reads that file at test time and
 * asserts every value here equals it, so a version bump that changes a token
 * is caught rather than silently drifting.
 *
 * Namespaces transcribed: `--spacing`, `--color-*`, `--text-*` (font size)
 * with their `--text-*--line-height` companions, `--font-weight-*`,
 * `--font-{sans,serif,mono}` (family), `--leading-*`, `--tracking-*`,
 * `--radius-*`, `--container-*`. Every other namespace in `theme.css`
 * (shadows, blur, breakpoints, animation, aspect ratio, perspective) is out
 * of scope for the core utility set this tool converts.
 */
...
export const THEME = {
  ...
  /** Font size, with each size's own line-height companion (`--text-*--line-height`). */
  text: {
...
export type ColorKey = keyof typeof THEME.color;
export type ContainerKey = keyof typeof THEME.container;
export type TextKey = keyof typeof THEME.text;
export type FontWeightKey = keyof typeof THEME.fontWeight;
export type FontFamilyKey = keyof typeof THEME.fontFamily;
export type TrackingKey = keyof typeof THEME.tracking;
export type LeadingKey = keyof typeof THEME.leading;
export type RadiusKey = keyof typeof THEME.radius;

/**
 * Reconstructs the exact `--custom-property: value` pairs this table was
 * transcribed from, keyed the same way `theme.css` names them. Used only by
 * the theme-equality test, so the test compares against the same data this
 * module actually exports rather than a second, independently-typed copy
 * that could silently drift from it.
 */
export function themeCssVariables(): Record<string, string> {
  const out: Record<string, string> = {};
  out['--spacing'] = THEME.spacing;
  for (const [key, value] of Object.entries(THEME.color)) out[`--color-${key}`] = value;
  for (const [key, value] of Object.entries(THEME.container)) out[`--container-${key}`] = value;
  for (const [key, value] of Object.entries(THEME.text)) {
    out[`--text-${key}`] = value.fontSize;
    out[`--text-${key}--line-height`] = value.lineHeight;
  }
  for (const [key, value] of Object.entries(THEME.fontWeight)) out[`--font-weight-${key}`] = value;
  for (const [key, value] of Object.entries(THEME.fontFamily)) out[`--font-${key}`] = value;
  for (const [key, value] of Object.entries(THEME.tracking)) out[`--tracking-${key}`] = value;
  for (const [key, value] of Object.entries(THEME.leading)) out[`--leading-${key}`] = value;
  for (const [key, value] of Object.entries(THEME.radius)) out[`--radius-${key}`] = value;
  return out;
}
