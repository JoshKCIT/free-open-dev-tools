...
also runs in CI on every push and pull request, reading its list from a repository variable
instead — that is the enforcement; the local hooks are a convenience so you find out sooner.

A fork's CI has no repository variable of that name, so the gate fails closed there by design. A
fork maintainer who wants CI to pass again must either set their own repository variable or remove
