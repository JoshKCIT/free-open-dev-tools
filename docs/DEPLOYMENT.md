...
7. Go to **Settings, Secrets and variables, Actions, Variables**, and add a repository variable named
   `PROVENANCE_DENYLIST` holding the forbidden-reference list, one entry per line or comma separated. The release
   gate (`scripts/check-provenance.mjs`) and the post-deployment check below both read this variable, and both fail
   the build when it is empty in CI, by design — a fork of this repository has no such variable and therefore fails
   closed until its own maintainer sets one.
...
| Repository variables        | Settings, Secrets and variables, Actions | **None.** `PROVENANCE_DENYLIST` had never been set                                                                                             |
...
- Either the served HTML (the prerendered page descriptions, which live in the HTML rather than the bundle) or the
  served JavaScript carries a forbidden reference from the `PROVENANCE_DENYLIST` repository variable.
