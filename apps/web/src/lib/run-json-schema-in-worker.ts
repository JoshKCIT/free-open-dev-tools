/**
 * The page-side helper that starts the schema-validating worker and races
 ...
 * same `setTimeout` race that is the only way to stop a runaway schema
 * validation -- a `pattern`/`patternProperties` keyword compiles to a
 * native regular expression match (Ajv's own keyword implementation,
 * `new RegExp(pattern, 'u').test(value)` by default per its
 * `unicodeRegExp` option), which is synchronous and un-interruptible from
 * inside once started, for the identical reason `run-regex-in-worker.ts`
 * gives for a bare pattern match. So this function, not the worker, owns
 * the timer, and composes the time-limit message itself when it wins the
 * race.
 */
import SchemaWorker from './workers/json-schema-validator.worker.ts?worker&inline';
import type { SchemaJobMessage, SchemaWorkerMessage } from './workers/json-schema-validator.worker';
...
export const SCHEMA_TIME_LIMIT_MESSAGE =
  'Stopped after 1.5 seconds: validation took too long. A pattern in the schema may backtrack catastrophically. Try a more specific pattern or smaller data.';

/**
 * Carries the position and kind a schema or data error was found at across
 * the worker boundary -- postMessage cannot clone a SchemaValidatorError
 ...
 */
export class SchemaRunError extends Error {
  readonly kind: 'schema-json' | 'data-json' | 'schema' | 'draft' | 'unknown';
  readonly line?: number;
  readonly column?: number;

  constructor(message: string, kind: SchemaRunError['kind'], line?: number, column?: number) {
    super(message);
    this.name = 'SchemaRunError';
    this.kind = kind;
    this.line = line;
    this.column = column;
  }
...
/**
 * Runs one schema-validation job in the background worker, resolving with
 * its result.
 *
 ...
 */
export function jsonSchemaInWorker(job: SchemaJobMessage, ctx: RunContext): Promise<ValidateJsonResult> {
  ...
  return new Promise<ValidateJsonResult>((resolve, reject) => {
    ...
    type Outcome = { ok: true; value: ValidateJsonResult } | { ok: false; error: Error };
    ...
    const onMessage = (event: MessageEvent<SchemaWorkerMessage>) => {
      const data = event.data;
      if (data.type === 'json-schema-done') settle({ ok: true, value: data.result });
      else settle({ ok: false, error: new SchemaRunError(data.message, data.kind, data.line, data.column) });
    };
    ...
    const timer = setTimeout(() => {
      settle({ ok: false, error: new Error(SCHEMA_TIME_LIMIT_MESSAGE) });
    }, SCHEMA_TIME_LIMIT_MS);
