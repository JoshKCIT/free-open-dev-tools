/**
 * The page-side helper that starts the Compose-validating worker and races
 ...
 * privacy-harness reason those files give: the worker is constructed when
 * the visitor presses Run (or types), and the inlining form embeds the
 * worker's code in the page chunk instead of emitting a separately fetched
 * file. The settlement contract is copied wholesale: one guarded `settle`
 * closure, the same fixed failure message on every native-failure path, and
 * the same `setTimeout` race that is the only way to stop a runaway
 * validation -- a pathological YAML document's duplicate-key scan is
 * synchronous and un-interruptible from inside once started, so this
 * function, not the worker, owns the timer.
 */
import ComposeWorker from './workers/docker-compose-validator.worker.ts?worker&inline';
import type { ComposeJobMessage, ComposeWorkerMessage } from './workers/docker-compose-validator.worker';
...
/**
 * Carries the position a Compose validation error was found at across the
 * worker boundary -- postMessage cannot clone a `YamlSourceError`/
 * `ComposeValidatorError` instance with its prototype intact, so the worker
...
/**
 * Runs one Compose-validation job in the background worker, resolving with
 * its result.
 *
 ...
 */
export function composeInWorker(job: ComposeJobMessage, ctx: RunContext): Promise<ComposeValidateResult> {
  if (ctx.signal.aborted) {
    return Promise.reject(new Error('The run was cancelled before it started.'));
  }

  return new Promise<ComposeValidateResult>((resolve, reject) => {
    ...
    type Outcome = { ok: true; value: ComposeValidateResult } | { ok: false; error: Error };
    ...
    const onMessage = (event: MessageEvent<ComposeWorkerMessage>) => {
      const data = event.data;
      if (data.type === 'compose-done') settle({ ok: true, value: data.result });
      else settle({ ok: false, error: new ComposeRunError(data.message, data.line, data.column, data.path) });
    };
    ...
    const timer = setTimeout(() => {
      settle({ ok: false, error: new Error(COMPOSE_TIME_LIMIT_MESSAGE) });
    }, COMPOSE_TIME_LIMIT_MS);
